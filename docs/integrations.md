...
| System | Used for | Status here |
|---|---|---|
| PMS (reads only) | the Warden's rate cross-check - the published rate for the room type and dates a draft quotes | `mock`/`csv` universal; `cloudbeds` built |
| Email | the Notary's inbox intake and the delivery/acknowledgement email on `signoff` | `mock`/`imap` universal; `gmail` built |
| Messaging | the duty-manager alert on a block/escalation/release, and the SLA-breach alert from `tools/gdpr.py sweep` | `mock`/`webhook` universal; `unipile` built |
| Sheets | not written to by this repo's own tools yet - reserved for a future audit-trail export; `make doctor` still reports it | `csv` universal; `google` built |
| POS, Accounting, Reviews, Calendar, Payments, Procurement, Locks, Courier | not used | stub |

The Warden never writes to the PMS and never sends a guest-facing message
itself - see `docs/how-it-works.md`. The **draft feed** (what the Warden
screens) is not one of the four families above; it is this agent's own
concept, documented at the end of this page.
...
## The draft feed (`config/agent.yaml: draft_feed`)

This is not one of the four adapter families above - it is how another
agent hands the Warden a draft to screen, and it is deliberately the
simplest thing that works: a JSON file per draft.

| Adapter | Status | Needs | Notes |
|---|---|---|---|
| `mock` (default) | universal | nothing | Reads `fixtures/inbound/drafts/*.json`. What `make demo` and the tests use. |
| `dir` | universal | a drop folder | Reads every `draft-*.json` in `draft_feed.dir` (default `data/imports/drafts`). |

A draft record looks like `fixtures/inbound/drafts/draft-01-rate-mismatch.json`:

```json
{
  "id": "draft-01", "agent": "Front Desk AI", "channel": "email",
  "guest": "T. Solberg", "category": "question", "confidence": 54,
  "quoted_price": 212, "room_type_id": "deluxe-sea-view",
  "check_in": "2026-08-14", "check_out": "2026-08-16", "claims": [],
  "draft_text": "...", "received_at": "2026-08-27T08:00:00+00:00"
}
```

**To wire up a real one.** Whatever agent drafts guest replies for you
writes one such file to `draft_feed.dir` right before it would otherwise
call `send()`, then waits for the Warden's verdict (poll `python3
tools/review.py show <id>`, or check whether the item reached `auto_sent`)
before actually sending. `category` should be one of the values in
`config/agent.yaml: category_blocks.categories` when it applies; `claims`
is a list of factual assertions worth checking against
`knowledge/allergens.md`. There is no live cross-repo call in this factory

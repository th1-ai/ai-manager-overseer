...
| `allergens.md` | The dish facts `tools/engine.py:check_allergen()` checks a draft's claims against - the Warden's highest-severity check. Matters more here than in most repos: see `docs/safety.md`. |
| `gdpr-intake-phrases.md` | The phrases `tools/gdpr.py:classify_intake()` uses to recognise a GDPR-shaped email, and which of the three kinds - see `docs/sub-agents.md`. Ships in English plus es/fr/de/it/pt, accent-folded. |
| `retention-policy.md` | The human-readable explanation behind `config/agent.yaml: gdpr.retention_holds` - keep the two in sync. |

...
`test_overseer_*.py` must never read this repo's own `config/hotel.yaml` /
`config/agent.yaml` - those are the hotel's own edits, and a real property
filling them in must never be able to turn `make test` red. See
...
test, so this fixture only isolates the overseer-specific test files -
...
def isolated_config(request, tmp_path, monkeypatch):
    ...
    if not module_file.startswith("test_overseer_"):
        yield
        return
